...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  ...
  const config: Record<string, { label: string; className: string }> = {
    ...
    INACTIVE: { label: "Inactive", className: "bg-obsidian-100 text-obsidian-600 border-obsidian-200" },
    SUSPENDED: { label: "Suspended", className: "bg-red-100 text-red-700 border-red-200" },
    PENDING: { label: "Pending", className: "bg-brand-orange/10 text-brand-orange border-brand-orange/20" },
  ...
  const current = config[s] || { label: status, className: "bg-surface-base text-obsidian-600 border-surface-sunken" };
