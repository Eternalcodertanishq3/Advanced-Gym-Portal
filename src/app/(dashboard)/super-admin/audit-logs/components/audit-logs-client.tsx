  ...
  const columns: ColumnDef<AuditLog>[] = [
    {
      ...
      header: "Timestamp",
      cell: ({ row }) => <div className="text-white/60 font-mono text-[10px]">{row.getValue("timestamp")}</div>,
    },
    {
      accessorKey: "user",
      header: "User / Actor",
      cell: ({ row }) => <div className="font-medium text-white text-sm">{row.getValue("user")}</div>,
    },
    {
      accessorKey: "action",
      header: "Action Performed",
      cell: ({ row }) => <div className="text-white/80 text-xs">{row.getValue("action")}</div>,
    },
    {
      accessorKey: "entity",
      header: "Target Entity",
      cell: ({ row }) => (
        <span className="px-2 py-1 rounded-md text-[10px] font-mono uppercase bg-white/5 text-white/70 border border-white/10">
          {row.getValue("entity")}
        </span>
      ),
    },
    ...
    <div className="space-y-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/50 mt-1">Immutable record of all administrative and system actions.</p>
        </div>
