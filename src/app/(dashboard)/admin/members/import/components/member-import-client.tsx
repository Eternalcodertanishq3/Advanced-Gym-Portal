...
export function MemberImportClient({
  ...
  const processData = async (data: any[]) => {
    ...
    if (!hasMandatory) {
      toast.error(
        "Could not detect mandatory columns (First Name, Last Name, Email). Please check your file headers.",
      );
      setIsProcessing(false);
      return;
    }
    ...
    try {
      ...
      if (response.success && response.results) {
        ...
        setProgress(100);
        toast.success(
          `Import complete: ${response.results.success} added, ${response.results.duplicates} skipped`,
        );
      } else {
  ...
  const downloadTemplate = () => {
    const headers =
      "firstName,lastName,email,phone,gender,dateOfBirth,bloodGroup,address,city,state,pincode\n";
    const sample =
      "John,Doe,john@example.com,9876543210,MALE,1990-01-01,O+,123 Main St,Mumbai,Maharashtra,400001\n";
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              {branches.length > 0 && (
                <div className="flex items-center gap-2">
                  <Label htmlFor="branch-select" className="whitespace-nowrap text-xs font-bold">
                    Target Branch:
                  </Label>
                  <Select value={selectedBranchId} onValueChange={setSelectedBranchId}>
                    <SelectTrigger id="branch-select" className="h-9 w-[200px] bg-muted/30">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <Card className="border-border/50 bg-card/50">
          ...
          <CardContent className="space-y-4 text-sm">
            <div className="flex items-start gap-3">
              ...
              <p>
                <span className="font-semibold text-foreground">firstName, lastName, email</span>{" "}
            ...
            <div className="flex items-start gap-3">
              ...
              <p>
                phone, gender (MALE/FEMALE), dateOfBirth (YYYY-MM-DD), address, city, state,
                pincode.
              </p>
            </div>
            <div className="flex items-start gap-3">
        ...
        <Card className="border-border/50 bg-card/50">
          ...
          <CardContent className="space-y-4 text-sm">
            <p>
              1. Members can log in using their email and their randomly generated temporary
              password.
            </p>
            <p>
              2. They will be prompted to update their profile and set a custom password on their
              first login.
            </p>
