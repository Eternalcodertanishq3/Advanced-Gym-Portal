...
import { PackageSearch, Plus, Search, Filter, ShoppingCart, ArrowUpDown, MoreHorizontal, Edit, Package } from "lucide-react";
...
export default function InventoryPage() {
  ...
  const getStockStatus = (quantity: number) => {
    if (quantity === 0) return { label: "Out of Stock", badge: "bg-red-100 text-red-800 border-red-200" };
    if (quantity < 10) return { label: "Low Stock", badge: "bg-orange-100 text-orange-800 border-orange-200" };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" className="bg-surface-card border-surface-sunken" onClick={() => router.push("/admin/inventory/pos")}>
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-surface-base">
              <TableRow>
                <TableHead className="font-semibold text-obsidian-900 w-16">Icon</TableHead>
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} className="h-32 text-center text-obsidian-500">
                    <PackageSearch className="w-8 h-8 text-obsidian-300 mx-auto mb-2" />
                    No products found.
                  </TableCell>
                </TableRow>
              ) : (
                    ...
                    <TableRow key={item.id} className="hover:bg-surface-base/50 transition-colors">
                      <TableCell>
                        <div className="w-10 h-10 rounded-lg bg-surface-base border border-surface-sunken flex items-center justify-center">
                          <Package className="w-5 h-5 text-obsidian-500" />
                        </div>
                      </TableCell>
                      <TableCell>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-obsidian-500">
